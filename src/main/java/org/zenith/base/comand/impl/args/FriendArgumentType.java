...
public class FriendArgumentType implements ArgumentType<String> {
   ...
   public String parse(StringReader var1) throws CommandSyntaxException {
      ...
      if (!ZenithClient.on23().MediaTrackInfo().isFriend(s)) {
         throw new DynamicCommandExceptionType(var1x -> Text.literal("У тебя нет друга " + s)).create(s);
      } else {
         return s;
      }
   }
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> var1, SuggestionsBuilder var2) {
      return CommandSource.suggestMatching(ZenithClient.on23().MediaTrackInfo().getItems(), var2);
   }
